...
import com.example.util.login.EmailUtil;
...
public class EmailServiceImpl  { //implements UserService
    ...
    public Result<?> emailLogin(String email, String password){
        ...
        try{
            StpUtil.login(res.getUserId());//调用session的登录接口，生成session
            SaSession session = StpUtil.getSession();

            session.set("name", res.getUserName());//从 User-Session 中读取、写入数据
        ...
        if (res != null){
            ...
            if (userVo.getPassword() == null) {
                ...
                if (emailUtil.registered(userVo)){ //验证邮箱
                    userVo.setPassword(SecureUtil.md5(userVo.getPassword()));// TODO: MD5+salt加密
                    User user = UserVoToUser.toUser(userVo);
                    user.setUserId(YitIdHelper.nextId());
                    userMapper.insert(user);//TODO: 用mp实现
                    return Result.success();
                }else {
